            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                <FieldSet>
                    ...
                    <section className="px-4">
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="emoji" className="text-[14px]">Emoji</Label>
                            <div className="flex flex-wrap gap-4">
                                {
                                    EMOJI_OPTIONS.map(({ emoji, label }) => (
                                        <Button role="radio" id="emoji" key={label} type="button" aria-pressed={emoji === selectedEmoji ? "true" : "false"} aria-label={label} title={label} onClick={() => setValue("emoji", emoji)} className={
                                            cn(
                        ...
                        <div className="pt-2">
                            <Label htmlFor="channel" className="text-[14px]">Channel</Label>
                            <span className="text-[12px] text-zinc-500 block pb-3 pt-1">Where to receive the notification</span>
                            <div className="flex flex-wrap gap-4">
                                {
                                    CHANNELS.map(({ name, icon }) => (
                                        <Button role="checkbox" id="channel" key={name} type="button" aria-pressed={selectedChannel.includes(name) ? "true" : "false"} aria-label={name} onClick={() => setValue("channels", (() =>
